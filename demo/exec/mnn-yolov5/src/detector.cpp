...
Detector::Detector(Config &config)
{
    ...
    this->net = std::shared_ptr<MNN::Interpreter>(MNN::Interpreter::createFromFile(config.weightPath));
    if (nullptr == this->net) {
    ...
    this->session = this->net->createSession(ScheduleConfig);

    this->inSize = config.size;
    this->_auto = config._auto;    
}

PadInfo Detector::letterbox(Mat &img, cv::Size new_shape, cv::Scalar color, bool _auto, bool scaleFill, bool scaleup, int stride)
{
...
Detection Detector::detect(const char* inputImageFileName)
{

    this->img = imread(inputImageFileName, IMREAD_COLOR);
    int originalWidth=this->img.cols;
    int originalHeight=this->img.rows;

    LOG_F(INFO, "img w:%d, h:%d\n", originalWidth, originalHeight);

    // this->img=cv::Mat(originalHeight, originalWidth, CV_32FC3, this->inputImage);
    // imshow("origin", this->img);
    // waitKey(0);
    ...
    Mat in_img = dnn::blobFromImage(im, 1 / 255.0f, Size(im.cols, im.rows), Scalar(0, 0, 0), true, false);
    std::vector<Mat> outs;

    MNN::Tensor* inputTensor = this->net->getSessionInput(this->session, nullptr);
    MNN::Tensor* inHostTensor = MNN::Tensor::create<float>(inputTensor->shape(), nullptr, MNN::Tensor::CAFFE);
    for (size_t i = 0; i < inHostTensor->elementSize(); i++)
    {
        inHostTensor->host<float>()[i] = in_img.ptr<float>()[i];
    }
    // inHostTensor->print();
    inputTensor->copyFromHostTensor(inHostTensor);
    MNN::Tensor::destroy(inHostTensor);

    // run network
    // AUTOTIME;
    this->net->runSession(session);
    // get output data
    MNN::Tensor *tensor_scores  = net->getSessionOutput(session, "output0");
    MNN::Tensor tensor_scores_host(tensor_scores, tensor_scores->getDimensionType());
    tensor_scores->copyToHostTensor(&tensor_scores_host);

    int matSize[] = {tensor_scores_host.shape()[0],tensor_scores_host.shape()[1],tensor_scores_host.shape()[2]};
    Mat out=Mat(3, matSize, CV_32F);
    for (size_t i = 0; i < tensor_scores_host.elementSize(); i++)
    {
        out.ptr<float>()[i]=tensor_scores_host.host<float>()[i];
    }
    outs.push_back(out);
...
void Detector::postProcess(Detection &detection, Colors &cl)
{
    PadInfo padInfo = letterbox(this->img, this->inSize, Scalar(114, 114, 114), this->_auto, false, true, 32);
    ...
    drawPredection(this->img, boxes, scores, clsIndexs, indices, cl);
    // imshow("rst", img);
    // waitKey(0);
    imwrite("output.jpg", this->img);
}
...
void Detector::drawPredection(Mat &img, std::vector<Rect> &boxes, std::vector<float> &scores, std::vector<int> &clsIndexs, std::vector<int> &ind, Colors &cl)
{
    ...
    for (int i = 0; i < ind.size(); i++)
    {
        ...
        LOG_F(INFO, "put label:%s", label.c_str());

        int baseLine = 0;
        Size textSize = getTextSize(label, FONT_HERSHEY_PLAIN, 1, 1, &baseLine);
        baseLine += 2;
        rectangle(img, Rect(rect.x, rect.y - textSize.height, textSize.width + 1, textSize.height + 1), color, -1);
        putText(img, label, Point(rect.x, rect.y), FONT_HERSHEY_PLAIN, 1, Scalar(255, 255, 255), 1);
    }
}
